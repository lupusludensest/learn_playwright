  ...
  async _runTest(test, retry, nextTest) {
    ...
    if (testInfo._isFailure()) {
      ...
      if (!process.env.PW_DISABLE_FLOATING_PROMISES_WARNING && testInfo._floatingPromiseScope.hasFloatingPromises()) {
        const annotationLocations = new Map(testInfo._floatingPromiseScope.floatingPromises().map(
          ({ location }) => {
            const locationKey = location ? `${location.file}:${location.line}:${location.column}` : void 0;
            return [locationKey, location];
          }
        ));
        testInfo.annotations.push(...[...annotationLocations.values()].map((location) => ({
          type: "warning",
          description: `This async call was not awaited by the end of the test. This can cause flakiness. It is recommended to run ESLint with "@typescript-eslint/no-floating-promises" to verify.`,
          location
        })));
